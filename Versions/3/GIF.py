...
class EnhancedGifCreatorThread(QThread):
    ...
    def __init__(self, image_paths, output_path, fade_steps, hold_duration, fade_duration, 
                 preserve_quality=False, quality=95, resampling_method="LANCZOS (Best)", crop_area=None):
        super().__init__()
...
class GifFadeCreator:
    ...
    def resize_images_to_match(self, images, target_size=None, preserve_quality=False, resampling_filter=Image.Resampling.LANCZOS):
        ...
        for img in images:
            ...
            img_resized = img.resize((new_width, new_height), resampling_filter)
    ...
    def create_fade_gif(self, image_paths, output_path, fade_steps=15, hold_duration=500, 
        ...
        # Convert to RGB with better quality preservation
        rgb_frames = []
        for frame in all_frames:
            if preserve_quality and frame.mode == 'RGBA':
                # Better RGBA to RGB conversion
                rgb_frame = Image.new('RGB', frame.size, (255, 255, 255))
                rgb_frame.paste(frame, mask=frame.split()[-1])
            else:
